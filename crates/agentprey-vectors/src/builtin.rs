...
static BUILTIN_VECTORS_DIR: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../../cli/vectors");
...
mod tests {
    ...
    fn loads_embedded_vector_catalog() {
        ...
        assert!(loaded
            .iter()
            .any(|vector| vector.vector.id == "tm-openclaw-004"));
        assert!(loaded
            .iter()
            .any(|vector| vector.vector.id == "tm-openclaw-005"));
        assert!(loaded
            .iter()
            .any(|vector| vector.vector.id == "tm-openclaw-006"));
        assert!(loaded
            .iter()
            .any(|vector| vector.vector.id == "tm-openclaw-007"));
        assert!(loaded
            .iter()
            .any(|vector| vector.vector.id == "ab-openclaw-001"));
        assert!(loaded
            .iter()
